...
function EditaProduto() {
  ...
  const editProduto = async () => {
    ...
    const endpoint = `${host}/api/v1/product/${id}`;
  ...
  const getParams = async () => {
    ...
    const item = JSON.parse(await AsyncStorage.getItem('item'));
    console.log(item);
  ...
  return (
    <ScrollView style={{backgroundColor:'white'}}>
      <Nav onPress={() => navigation.navigate("ExibeProdutosAdmin")} />
      <View style={styles.container}>
        <Statusbar />

        <Text style={styles.texttop}>Edição do Produto </Text>      
      ...
      <TextInput
        ...
        <TouchableOpacity style={styles.button} onPress={editProduto}>          
